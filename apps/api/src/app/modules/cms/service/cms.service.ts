...
export class CMSService {
  ...
  async getSummary(input: GetSummaryDto) {
    try {
      ...
      const cachedSummary = await this.redisService.getKey(cacheKey);
      if (cachedSummary) {
      ...
      await this.redisService.setKeyObject(cacheKey, responseResult, 600);
  ...
  async getSummaryAll() {
    try {
      const resultSummary = await this.redisService.getKey(`summary-cms`);
